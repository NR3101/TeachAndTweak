...
const Modern = ({ lessonPlan }: { lessonPlan: LessonPlanWithSections }) => {
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      >
        ...
        <div className="flex flex-wrap gap-4">
          <Badge
            variant="secondary"
            className="text-sm py-1 px-2 dark:text-white"
          >
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {lessonPlan.sections?.map((section: Section, index: number) => (
          ...
          >
            <Card className="h-full flex flex-col bg-card dark:bg-card/80 transition-colors duration-300">
              <CardHeader>
                <CardTitle className="text-xl font-semibold text-foreground dark:text-primary">
                  {section.title}
                </CardTitle>
                <Badge
                  variant="outline"
                  className="w-fit text-foreground border-foreground/20"
                >
